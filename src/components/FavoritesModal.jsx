    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
        <motion.div
          ...
          className="w-full max-w-xl glass-panel rounded-3xl p-6 shadow-2xl border border-slate-700/80 relative flex flex-col max-h-[85vh]"
        >
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto py-4 space-y-3">
            {favorites.length === 0 ? (
              <div className="text-center py-10 space-y-3">
                ...
                <p className="text-sm text-slate-400">No favorited recipes saved yet.</p>
              ...
              favorites.map((recipe, idx) => (
                <div
                  key={recipe.id || idx}
                  className="glass-card rounded-2xl p-4 border border-slate-700/60 flex items-center justify-between gap-3 hover:border-indigo-500/50 transition-all group"
                >
                  <div
                    onClick={() => {
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-indigo-600 text-white font-bold text-xs shadow-md hover:bg-indigo-500 transition-all cursor-pointer flex items-center gap-1"
                    >
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-slate-900/80 text-rose-400 hover:text-rose-300 border border-slate-800 transition-all cursor-pointer"
